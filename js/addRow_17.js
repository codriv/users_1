...
function leker() {
    fetch("http://ifksp-prod-probadocker-yr4i9t.mo5.mogenius.io:80/users", fetchInitGet).then(
        data => data.json(),
        err => console.log(err)
        ).then(
        userek => {
...
function rowDelFetch (sorszam) {
    fetch(`http://ifksp-prod-probadocker-yr4i9t.mo5.mogenius.io:80/users/${sorszam}`, fetchInitDelete).then(
...
function rowModFetch (sorszam, inputok, gombWarning) {
    ...
    fetch(`http://ifksp-prod-probadocker-yr4i9t.mo5.mogenius.io:80/users/${sorszam}`, fetchInitPatch).then(
        resolve => resolve.json(),
        rejet => console.error(reject)
    ).then(
        data => {
...
function addRowFetch (newUser) {
    ...
    fetch("http://ifksp-prod-probadocker-yr4i9t.mo5.mogenius.io:80/users", fetchInitPost).then(
        resolve => resolve.json(),
        rejet => console.error(reject)
    ).then(
        data => {
